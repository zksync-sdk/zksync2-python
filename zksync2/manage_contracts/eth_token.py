...
class EthToken:
    ...
    def __init__(
        ...
        self.contract = self.module.contract(
            self.contract_address, abi=_eth_token_abi_default()
        )
        self.account = account

    def _nonce(self) -> int:
        return self.module.get_transaction_count(
            self.account.address, EthBlockParams.LATEST.value
        )

    def withdraw_tx(self, to: HexStr, amount: int, gas: int, gas_price: int = None):
