...
class L1Bridge:
    ...
    def claim_failed_deposit(
        ...
        self.contract.functions.claimFailedDeposit(
            deposit_sender,
            l1_token,
            l2tx_hash,
            l2_block_number,
    ...
    def deposit(
        ...
        amount: int,
        l2_tx_gas_limit: int,
        l2_tx_gas_per_pubdata_byte: int,
    ) -> TxReceipt:
    ...
    def finalize_withdrawal(
        ...
        tx = self.contract.functions.finalizeWithdrawal(
            l2_block_number, l2_msg_index, msg, merkle_proof
        ).build_transaction(
            {
                "chainId": self.web3.eth.chain_id,
                "from": self.account.address,
    ...
    def is_withdrawal_finalized(self, l2_block_number: int, l2_msg_index: int) -> bool:
        return self.contract.functions.isWithdrawalFinalized(
            l2_block_number, l2_msg_index
        ).call()
