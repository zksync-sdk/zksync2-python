...
class LegacyContractFactory:
    ...
    def _deploy_create(
        self, salt: bytes = None, deps: List[bytes] = None, *args: Any, **kwargs: Any
    ) -> Contract:
    ...
    def _deploy_create_account(
        ...
        create2_contract = TxCreateAccount(
            web3=self.web3,
            chain_id=self.web3.zksync.chain_id,
            nonce=nonce,
            from_=self.account.address,
    ...
    def deploy(
        ...
        if self.type == DeploymentType.CREATE2:
            return self._deploy_create2(salt, deps, *args, **kwargs)
        if self.type == DeploymentType.CREATE_ACCOUNT:
            return self._deploy_create_account(salt, deps, *args, **kwargs)
        if self.type == DeploymentType.CREATE2_ACCOUNT:
            return self._deploy_create2_account(salt, deps, *args, **kwargs)
        return self._deploy_create(salt, deps, *args, **kwargs)
