...
from zksync2.core.types import ZkBlockParams, L2BridgeContracts, TransferTransaction, TransactionOptions, \
    WithdrawTransaction, ADDRESS_DEFAULT
from zksync2.core.utils import is_eth
from zksync2.manage_contracts.deploy_addresses import ZkSyncAddresses
from zksync2.manage_contracts.utils import zksync_abi_default, nonce_holder_abi_default, l2_bridge_abi_default, \
    get_erc20_abi
...
class WalletL2:
    def __init__(self,
        ...
        self.contract = self._eth_web3.eth.contract(Web3.to_checksum_address(self._main_contract_address), abi=zksync_abi_default())
    ...
    def transfer(self, tx: TransferTransaction) -> HexStr:
        tx_fun_call = self._zksync_web3.zksync.get_transfer_transaction(tx, self._l1_account.address)
        ...
        if tx.token_address is None or is_eth(tx.token_address):
            ...
            transaction = token_contract.functions.transfer(tx.to, tx.amount).build_transaction(
                prepare_transaction_options(options,
    ...
    def withdraw(self, tx: WithdrawTransaction):
        ...
        if tx.options.nonce is None:
            tx.options.nonce = self._zksync_web3.zksync.get_transaction_count(Web3.to_checksum_address(self._l1_account.address), ZkBlockParams.LATEST.value)
        ...
        if tx.bridge_address is None:
            ...
            try:
                l2_weth_token = self.get_l2_bridge_contracts().weth.functions.l1TokenAddress(tx.token).call()
            except:
                pass
        ...
        bridge = self._zksync_web3.zksync.contract(address=Web3.to_checksum_address(tx.bridge_address),
                                                   abi=l2_bridge_abi_default())
        transaction = bridge.functions.withdraw(self._l1_account.address,
                                                tx.token,
                                                tx.amount).build_transaction(prepare_transaction_options(tx.options, self._l1_account.address))
