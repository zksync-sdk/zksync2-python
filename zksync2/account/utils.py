...
from zksync2.core.types import DepositTransaction, RequestExecuteCallMsg, TransactionOptions
from zksync2.transaction.transaction712 import Transaction712


def deposit_to_request_execute(transaction: DepositTransaction) -> RequestExecuteCallMsg:
...
def options_from_712(tx: Transaction712) -> TransactionOptions:
    return TransactionOptions(chain_id=tx.chain_id,
...
def prepare_transaction_options(options: TransactionOptions, from_: HexStr = None):
    ...
    if options.max_fee_per_gas is not None or options.max_priority_fee_per_gas is not None:
