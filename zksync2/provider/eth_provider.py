...
class EthereumProvider:
    ...
    def __init__(self, zksync_web3: Web3, eth_web3: Web3, l1_account: BaseAccount):
        ...
        self._main_contract = ZkSyncContract(
            zksync_main_contract=self._main_contract_address,
            eth=self._eth_web3,
            account=l1_account,
        )
        ...
        self._l1_bridge = L1Bridge(
            bridge_addresses.erc20_l1_default_bridge, self._eth_web3, l1_account
        )
    ...
    def get_l1_balance(self, token: Token, block: EthBlockParams):
        if token.is_eth():
            ...
            token_contract = ERC20Contract(
                self._eth_web3.eth, token.l1_address, self._l1_account
            )
    ...
    def get_base_cost(
        ...
        return self.main_contract.l2_tx_base_cost(
    ...
    def deposit(
        ...
        if bridge_address is not None:
            bridge_contract = L1Bridge(bridge_address, self._eth_web3, self._l1_account)
        ...
        if token.is_eth():
            ...
            if approve_erc20:
                self.approve_erc20(token, amount, bridge_address, gas_limit)
            tx_receipt = bridge_contract.deposit(
                l2_receiver=to,
    ...
    def request_execute(
        ...
        tx_receipt = self.main_contract.request_l2_transaction(
            contract_l2=contract_address,
            l2_value=l2_value,
            call_data=call_data,
            l2_gas_limit=l2_gas_limit,
    ...
    def finalize_withdrawal(self, withdraw_hash, index: int = 0):
        ...
        if is_eth(params["sender"]):
            ...
            l2bridge = L2Bridge(
                contract_address=params["sender"],
                web3_zks=self._zksync_web3,
                zksync_account=self._l1_account,
            )
            l1bridge = L1Bridge(
                contract_address=l2bridge.l1_bridge(),
                web3=self._eth_web3,
                eth_account=self._l1_account,
            )
            return l1bridge.finalize_withdrawal(
                l2_block_number=params["l1_batch_number"],
    ...
    def is_withdrawal_finalized(self, withdraw_hash, index: int = 0):
        ...
        if is_eth(sender):
            return self.main_contract.is_eth_withdrawal_finalized(
                l2_block_number=l2_block_number, l2_message_index=proof.id
            )
        else:
            # TODO: check should it be different account for L1/L2
            l2bridge = L2Bridge(
            ...
            return l1bridge.is_withdrawal_finalized(
                l2_block_number=l2_block_number, l2_msg_index=proof.id
            )
