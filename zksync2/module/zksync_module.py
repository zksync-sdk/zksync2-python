...
ZKSYNC_REQUEST_FORMATTERS: [RPCEndpoint, Callable[..., Any]] = {
    ...
    zks_estimate_gas_l1_to_l2_rpc: apply_formatter_at_index(zks_transaction_request_formatter, 0),
}
...
def to_batch_details(t: dict) -> BatchDetails:
    ...
    base_sys_contract_hashes = BaseSystemContractsHashes(bootloader=t["baseSystemContractsHashes"]["bootloader"],
                                                      default_aa=t["baseSystemContractsHashes"]["default_aa"])
    return BatchDetails(base_system_contracts_hashes=base_sys_contract_hashes,
...
def to_block_details(t: dict) -> BlockDetails:
    datetime_format = '%Y-%m-%dT%H:%M:%S.%fZ'
    return BlockDetails(commit_tx_hash = t["commitTxHash"],
                        committed_at = datetime.strptime(t["committedAt"], datetime_format),
                        execute_tx_hash = t["executeTxHash"],
                        executed_at = datetime.strptime(t["executedAt"], datetime_format),
                        l1_tx_count = t["l1TxCount"],
...
def to_transaction_details(t: dict) -> TransactionDetails:
    return TransactionDetails(ethCommitTxHash = t["ethCommitTxHash"],
...
def to_transaction_receipt(t: dict) -> TransactionReceipt:
    ...
    return TransactionReceipt(from_=t["from"],
                              to=t["to"],
...
def to_l2_to_l1_logs(t: dict) -> L1ToL2Log:
    return L1ToL2Log(block_hash=t["blockHash"],
                     block_number=t["blockNumber"],
...
def to_block_range(t: dict) -> BlockRange:
    return BlockRange(beginning=t["beginning"],
                      end=t["end"])
...
class ZkSync(Eth, ABC):
    ...
    _eth_get_transaction_receipt: Method[Callable[[HexStr], ZksTransactionReceipt]] = Method(
