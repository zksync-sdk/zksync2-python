...
class TxWithdraw(TxBase, ABC):
    def __init__(
        ...
        if token.is_eth():
            eth_l2_token = EthToken(
                web3=web3.zksync, contract_address=L2_ETH_TOKEN_ADDRESS, account=account
            )

            tx = eth_l2_token.withdraw_tx(
                to=to, amount=amount, gas=gas_limit, gas_price=gas_price
            )
        else:
            if bridge_address is None:
                bridge_addresses: BridgeAddresses = (
                    web3.zksync.zks_get_bridge_contracts()
                )
                bridge_address = bridge_addresses.erc20_l2_default_bridge
            l2_bridge = L2Bridge(
                contract_address=bridge_address, web3_zks=web3, zksync_account=account
            )
            tx = l2_bridge.withdraw_tx(
                l1_receiver=to,
