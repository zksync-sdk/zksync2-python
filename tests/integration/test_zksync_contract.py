...
class ZkSyncWeb3Tests(TestCase):
    ...
    def test_get_l2_bootloader_bytecode_hash(self):
        bytecode_hash = self.zksync_contract.functions.getL2BootloaderBytecodeHash().call(
            {
                "chainId": self.eth_web3.eth.chain_id,
                "from": self.account.address,
                'nonce': self.eth_web3.eth.get_transaction_count(self.account.address),
    ...
    def test_get_l2_default_account_bytecode_hash(self):
        bytecode_hash = self.zksync_contract.functions.getL2DefaultAccountBytecodeHash().call(
            {
                "chainId": self.eth_web3.eth.chain_id,
                "from": self.account.address,
                'nonce': self.eth_web3.eth.get_transaction_count(self.account.address),
    ...
    def test_request_l2_transaction(self):
        ...
        tx = contract.functions.requestL2Transaction(Web3.to_checksum_address(self.zksync.zksync.zks_main_contract()),
                                                                ...
                                                                [],
                                                                Web3.to_checksum_address(self.zksync.zksync.zks_main_contract())).build_transaction(
            {
                "nonce": self.eth_web3.eth.get_transaction_count(self.account.address),
                'from': self.account.address,
                "gasPrice": gas_price,
