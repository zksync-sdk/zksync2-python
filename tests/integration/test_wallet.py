...
from zksync2.core.types import Token, DepositTransaction, ADDRESS_DEFAULT, FullDepositFee, \
    RequestExecuteCallMsg, TransactionOptions, TransferTransaction, WithdrawTransaction, EthBlockParams
...
class TestWallet(TestCase):
    ...
    def test_get_address(self):
        ...
        self.assertEqual(address, "0x36615Cf349d7F6344891B1e7CA7C72883F5dc049", "Should return wallet address")
    ...
    def test_prepare_deposit_transaction(self):
        options = TransactionOptions(gas_price=1_000_000_007,
    ...
    def test_deposit_eth(self):
        ...
        tx_hash = self.wallet.deposit(DepositTransaction(token=Token.create_eth().l1_address,
                                            amount=amount))

        tx_receipt = self.eth_web3.eth.wait_for_transaction_receipt(tx_hash)
        l2_hash = self.zksync.zksync.get_l2_hash_from_priority_op(tx_receipt, self.zksync_contract)
    ...
    def test_deposit_token(self):
        amount = 5
        l1_address, l2_address = self.load_token()
        is_approved = self.wallet.approve_erc20(Web3.to_checksum_address(l1_address), amount)
        self.assertTrue(is_approved)

        balance_l2_beore = self.wallet.get_balance(token_address=Web3.to_checksum_address(l2_address))

        tx_hash = self.wallet.deposit(DepositTransaction(Web3.to_checksum_address(l1_address),
                                                            amount,
        ...
        balance_l2_after = self.wallet.get_balance(token_address=Web3.to_checksum_address(l2_address))
    ...
    def test_transfer_eth(self):
        amount = 7_000_000_000
        balance_before_transfer = self.zksync.zksync.get_balance(Web3.to_checksum_address(self.address2))
        tx_hash = self.wallet.transfer(TransferTransaction(to=Web3.to_checksum_address(self.address2),
        ...
        balance_after_transfer = self.zksync.zksync.get_balance(Web3.to_checksum_address(self.address2))
    ...
    def test_transfer_token(self):
        ...
        balance_before = self.zksync.zksync.zks_get_balance(self.address2, token_address=l2_address)
        tx_hash = self.wallet.transfer(TransferTransaction(to=Web3.to_checksum_address(self.address2),
        ...
        balance_after = self.zksync.zksync.zks_get_balance(self.address2, token_address=l2_address)
    ...
    def test_withdraw_token(self):
        ...
        withdraw_tx_hash = self.wallet.withdraw(WithdrawTransaction(Web3.to_checksum_address(l2_address), 5))

        self.zksync.zksync.wait_for_transaction_receipt(
            withdraw_tx_hash, timeout=240, poll_latency=0.5
        )

        l2_balance_after = self.wallet.get_balance(token_address=Web3.to_checksum_address(l2_address))
    ...
    def test_request_execute(self):
        ...
        tx_hash = self.wallet.request_execute(RequestExecuteCallMsg(contract_address=Web3.to_checksum_address(self.zksync.zksync.main_contract_address),
                                                                          call_data=HexStr("0x"),
                                                                          l2_value=amount,
